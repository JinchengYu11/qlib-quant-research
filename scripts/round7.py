"""
第 7 轮：CSI500 上的快速迭代。
A 部分：策略参数扫描（topk × n_drop × hold_thresh），训练一次复用
B 部分：Optuna 超参优化（Alpha158 on CSI500），看在小盘 universe 上是否依然不奏效
基线：CSI500 + Alpha158 + 默认超参 + n_drop=1 (净超额 +9.01%, IR 0.874)
"""
import warnings
...
def main():
    ...
    print("[1] 构建 Alpha158 CSI500 数据集 + 训练默认超参模型 ...", flush=True)
    handler = Alpha158(**DH)
    ...
    # === A. 策略参数扫描 ===
    print("\n[2] A 线：策略参数扫描（默认超参模型，扫 topk × n_drop × hold）...", flush=True)
    configs = [
        ("baseline", dict(topk=50, n_drop=1, hold_thresh=1)),
        ("T1-ndrop0", dict(topk=50, n_drop=0, hold_thresh=1)),
    ...
    pd.DataFrame(sweep).to_csv(OUT / "csi500_strategy_sweep.csv", index=False)

    # === B. Optuna 超参优化（看在小盘 universe 上是否依然过拟合） ===
    print(f"\n[3] B 线：Optuna 超参搜索 {N_TRIALS} 轮（CSI500, 目标=验证集 RankICIR）...", flush=True)
    ...
    print(f"\n[4] CSI500 最优超参（验证 RankICIR={study.best_value:.4f}）:", flush=True)
    ...
    print(f"\n[5] 调优超参 在 CSI500 上测试集回测 (n_drop=1):", flush=True)
    print(f"    净超额={m_tuned['excess_net_ann']:+.2f}%  IR={m_tuned['excess_ir']:+.3f}  净夏普={m_tuned['net_sharpe']:.3f}", flush=True)

    json.dump({"sweep": sweep, "tuned": m_tuned, "best_params": best,
               "valid_rankicir": float(study.best_value)},
              open(OUT / "round7_result.json", "w"), indent=1, ensure_ascii=False)

    # 总结
    print("\n=== CSI500 综合对比 ===", flush=True)
    base = next(s for s in sweep if s["name"] == "baseline")
    best_sweep = max(sweep, key=lambda s: s["excess_ir"])
    print(f"baseline (n_drop=1):      净超额={base['excess_net_ann']:+.2f}%  IR={base['excess_ir']:+.3f}", flush=True)
    print(f"最优策略参数 {best_sweep['name']}: 净超额={best_sweep['excess_net_ann']:+.2f}%  IR={best_sweep['excess_ir']:+.3f}", flush=True)
    print(f"调优超参 (n_drop=1):      净超额={m_tuned['excess_net_ann']:+.2f}%  IR={m_tuned['excess_ir']:+.3f}", flush=True)
    print(f"\n[DONE] 结果存 results/runs/round7/", flush=True)
