...
> **项目状态**：研究方法论闭环 + 7 反证 + R16 首次正向迁移（16 轮迭代）。
> R8.5 long-only IR 0.445 在 7 个改进方向上不被超过 (R9/R11/R13/R14/R15), 但 **R16 横向迁移到 CSI300
> 首次成功** — CSI300 IR 0.415 (≈ CSI500 0.445), MDD -12.6% (更好). 验证 R8.5 是 A 股大中盘通用策略.
> CSI300 LS Sharpe -0.256 是新发现 (大盘股做空 bottom K 反向, 套利充分).
> 详细 v2 报告见 [PROJECT_REPORT_v2.pdf](results/PROJECT_REPORT_v2.pdf), 日志见 [results/research_log.md](results/research_log.md)。
...
| **R16** | **R8.5 → CSI300 (横向迁移)** | **+3.87%** | **0.415** ✅ | **首次正向迁移! R8.5 是大中盘通用策略** |
